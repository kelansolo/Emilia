...
//       system("espeak \"press green to start\" -ven+f4 -s130 -a5 2>/dev/null &");
    ...
    case 10:
      ...
      snprintf(lines[line++], MAX_LEN, "vel=0.4, edger = 0 : dist=0.5");
      snprintf(lines[line++], MAX_LEN, "servo=3, pservo=800, vservo=0:time=0.3");
      snprintf(lines[line++], MAX_LEN, "vel=0.3, tr=0.15: turn=80.0");
      snprintf(lines[line++], MAX_LEN, "vel=0.5, edger=0.0: dist=0.3");
      snprintf(lines[line++], MAX_LEN, "vel=0.5, edger=0.0: ir1<0.5");
      snprintf(lines[line++], MAX_LEN, "vel=0.5: dist=0.70");
...
bool UMission::mission3(int & state)
{
  ...
  switch (state)
  {
     case 0:
      ...
      snprintf(lines[line++], MAX_LEN, "vel=0.5,acc=1,  edgel = 0 : ir1<0.5");
      snprintf(lines[line++], MAX_LEN, "vel=0.5, edgel = 0 : dist = 2");
      snprintf(lines[line++], MAX_LEN, "vel=0.5,  edgel = 0 : ir1<0.5");
